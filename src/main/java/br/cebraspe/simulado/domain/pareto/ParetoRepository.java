...
public class ParetoRepository {
    ...
    public List<ParetoAnalysis> findTopByContest(Long contestId, BigDecimal threshold) {
        return jdbcClient.sql("""
                SELECT pa.id, pa.topic_id, pa.contest_id, pa.incidence_rate,
                       ...
                       pa.last_updated,
                       t.name as topic_name, t.discipline
                ...
                .query((rs, n) -> new ParetoAnalysis(
                        rs.getLong("id"), rs.getLong("topic_id"),
    ...
    public void upsertUserPerformance(Long topicId, boolean isCorrect) {
        jdbcClient.sql("""
                INSERT INTO user_topic_performance (topic_id, total_answered,
                    total_correct, total_wrong, accuracy_rate, last_studied)
                VALUES (:topicId, 1, :correct, :wrong,
                        CASE WHEN :correctB THEN 1.0 ELSE 0.0 END, NOW())
                ON CONFLICT (topic_id) DO UPDATE SET
                    ...
                    accuracy_rate  = (user_topic_performance.total_correct + :correct)::numeric /
                                     (user_topic_performance.total_answered + 1),
                    last_studied   = NOW(),
                    mastery_level  = CASE
                        WHEN (user_topic_performance.total_correct + :correct)::numeric /
                             (user_topic_performance.total_answered + 1) >= 0.85 THEN 'AVANCADO'
                        WHEN (user_topic_performance.total_correct + :correct)::numeric /
                             (user_topic_performance.total_answered + 1) >= 0.65 THEN 'INTERMEDIARIO'
                        ELSE 'INICIANTE' END,
                    updated_at = NOW()
                """)
                .param("topicId", topicId)
                .param("correct", isCorrect ? 1 : 0)
                .param("wrong", isCorrect ? 0 : 1)
                .param("correctB", isCorrect)
                .update();
    }
    ...
    public record UserTopicPerformance(
            Long id, Long topicId, Integer totalAnswered, Integer totalCorrect,
            Integer totalWrong, BigDecimal accuracyRate,
            java.time.LocalDateTime lastStudied, String masteryLevel,
            java.time.LocalDateTime updatedAt) {
    }

    public record KeywordPerformance(
            Long id, Long keywordId, String keyword, String category,
            Integer wrongCount, Integer totalSeen, BigDecimal errorRate) {
    }
}